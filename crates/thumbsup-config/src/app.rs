...
#[derive(PartialEq, Eq)]
enum Tab { Settings, Registration, Cache, Diagnostics, About }
...
#[cfg(windows)]
fn load_settings() -> Settings { regio::load_settings() }
#[cfg(not(windows))]
fn load_settings() -> Settings { Settings::default() }

fn default_dll_path() -> String {
    // Best guess: alongside our exe.
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            return dir.join("thumbsup_shell.dll").to_string_lossy().into_owned();
        }
    }
...
impl ConfigApp {
    fn draw_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add(
            ...
            "EPUBs larger than this are skipped to keep Explorer responsive."
        );
    ...
    fn save_settings(&mut self) {
        ...
        match regio::save_settings(&self.settings) {
            Ok(_) => {
                ...
                self.status = "Settings saved. They take effect on the next thumbnail request.".into();
            }
    ...
    fn draw_diagnostics(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Open log folder").clicked() {
                #[cfg(windows)]
                if let Some(p) = diag::default_log_path().and_then(|p| p.parent().map(|p| p.to_path_buf())) {
