...
pub fn load_settings() -> Settings {
    ...
    if let Some(v) = read_qword(hkey, "MaxFileBytes") {
        s.max_file_mb = v / (1024 * 1024);
    }
    if let Some(v) = read_dword(hkey, "MaxThumbnailMs") {
        s.max_thumbnail_ms = v;
    }
    if let Some(v) = read_dword(hkey, "FallbackPolicy") {
        s.fallback_policy = FallbackPolicy::from_dword(v);
    }
    if let Some(v) = read_dword(hkey, "LoggingEnabled") {
        s.logging_enabled = v != 0;
    }
    if let Some(v) = read_string(hkey, "LogPath") {
        s.log_path = v;
    }
    unsafe {
...
fn read_dword(hkey: HKEY, name: &str) -> Option<u32> {
    ...
    if r == ERROR_SUCCESS && ty == REG_DWORD {
        Some(v)
    } else {
        None
    }
...
fn read_qword(hkey: HKEY, name: &str) -> Option<u64> {
    ...
    let r = unsafe {
        RegQueryValueExW(
            ...
            None,
            Some(&mut ty),
            Some(&mut v as *mut u64 as *mut u8),
            Some(&mut sz),
        )
    };
    if r == ERROR_SUCCESS && ty == REG_QWORD {
        Some(v)
    } else {
        None
    }
...
fn read_string(hkey: HKEY, name: &str) -> Option<String> {
    ...
    if r != ERROR_SUCCESS || ty != REG_SZ || sz == 0 {
        return None;
    }
...
fn write_dword(hkey: HKEY, name: &str, value: u32) -> std::io::Result<()> {
    ...
    let r = unsafe { RegSetValueExW(hkey, PCWSTR(n.as_ptr()), 0, REG_DWORD, Some(&bytes)) };
    if r != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(r.0 as i32));
    }
    Ok(())
...
fn write_qword(hkey: HKEY, name: &str, value: u64) -> std::io::Result<()> {
    let n = wide(name);
    let bytes = value.to_le_bytes();
    let r = unsafe { RegSetValueExW(hkey, PCWSTR(n.as_ptr()), 0, REG_QWORD, Some(&bytes)) };
    if r != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(r.0 as i32));
    }
    Ok(())
...
fn write_string(hkey: HKEY, name: &str, value: &str) -> std::io::Result<()> {
    ...
    let r = unsafe { RegSetValueExW(hkey, PCWSTR(n.as_ptr()), 0, REG_SZ, Some(bytes)) };
    if r != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(r.0 as i32));
    }
    Ok(())
