...
pub fn default_log_path() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(local).join("ThumbsUp").join("diagnostics.log"))
}
...
pub fn read_recent(path: &std::path::Path, max_entries: usize) -> std::io::Result<Vec<DiagEntry>> {
    ...
    let mut entries: Vec<DiagEntry> = content
        .lines()
...
fn parse_line(line: &str) -> Option<DiagEntry> {
    ...
    Some(DiagEntry { timestamp: ts, file, outcome, detail })
}
...
mod tests {
    ...
    fn parses_well_formed_line() {
        let entry = parse_line(
            "2026-01-15T12:34:56Z\tbook.epub\tok\tv3 strategy=epub3-cover-image",
        ).unwrap();
